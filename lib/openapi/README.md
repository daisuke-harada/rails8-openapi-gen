...
`lib/openapi/controller_generator.rb` は OpenAPI 仕様（resolved YAML）を解析して
...
3. `bundle exec rake openapi:generate_controllers` を実行してコントローラを更新する
...
lib/
├── openapi/
│   ├── controller_generator.rb   # ジェネレータ本体
│   └── README.md                 # このファイル
├── tasks/
│   └── generate_controllers.rake # rake タスク定義
└── templates/
    └── openapi/
...
# 特定リソースのみ生成したい場合
bundle exec rake openapi:generate_controllers[users]

# 複数リソースを指定する場合
bundle exec rake openapi:generate_controllers[users,posts]
```
...
3. `make gen` → `bundle exec rake openapi:generate_controllers` を実行する
